# -*- coding: utf-8 -*-
# main.py - CODIGO 100% CORRIGIDO PARA UBUNTU 22.04 + CHROMIUM 144.0.7559.132 (SNAP) - 1147 LINHAS
import sys
import os
import time
import json
...
class AviatorBotInteligenteV3:
    def __init__(self):
        ...
        # GERENCIAMENTO EMOCIONAL (MODULO 1)
        self.meta_diaria = 10.0
        self.stop_win = 15.0
        self.stop_loss = -5.0
        self.total_lucro = 0.0
        
        # SISTEMA DE AUTO-CORRECAO + REGISTRO DETALHADO
        self.memoria_erros = []
        ...
        # VARIAVEIS CRITICAS INICIALIZADAS
    ...
    def carregar_regras_auto_correcao(self):
        """Carregar regras de auto-correcao"""
    ...
    def carregar_acertos_erros(self):
        """Carregar historico de acertos e erros"""
    ...
    def salvar_padroes(self):
        """Salvar padroes no arquivo JSON"""
        try:
    ...
    def salvar_regras_auto_correcao(self):
        """Salvar regras de auto-correcao"""
        try:
    ...
    def salvar_acertos_erros(self):
        """Salvar historico de acertos e erros"""
        try:
    ...
    def fetch_data(self):
        """
        ✅ COLETA PRECISA DE DADOS - SEM NUMEROS GENERICOS!
        ✅ CORRECAO CRITICA: Usa Chromium 144.0.7559.132 do Snap (caminho: /snap/bin/chromium)
        ✅ User-Agent atualizado para Chrome/144.0.0.0
        ✅ Captura EXATA dos valores reais do grafico (1.137x, 1.147x, etc.)
        """
        try:
            with sync_playwright() as p:
                # ✅ CORRECAO CRITICA: Forcar uso do Chromium do Snap
                ...
                if isinstance(data, list):
                    # ✅ CORRECAO CRITICA: Linha 315 corrigida (faltava "data")
                    for item in data:
                        try:
                            num = float(item)
                            # ✅ CORRECAO CRITICA: Validação rigorosa (elimina 1.00x)
    ...
    def analisar_sequencias(self, dados):
        """Analisar sequencias com validacao rigorosa de valores numericos"""
        sequencias = []
    ...
    def detectar_padroes(self, sequencias):
        """Detectar padroes de 1-3 derrotas seguidas de vitoria"""
        novos_padroes = []
        sinais_imediatos = []
    ...
    def detectar_padroes_azul(self, dados):
        """Detecta padroes criticos no azul (1.00x-1.99x) do MODULO 1"""
    ...
    def detectar_padrao_xadrez(self, dados):
        """Detecta padroes de xadrez (positivo/negativo) do MODULO 1"""
    ...
    def analisar_estrategia_azuis(self, dados):
        """Analisa estrategia de azuis com base na decimal (MODULO 1: 1.09 = 9 reds)"""
    ...
    def detectar_estrategias_modo_2(self, dados):
        """Estrategias do MODULO 2: ROSA seguido de 5-6 AZULs"""
        if len(dados) < 6:
            return None
    ...
    def schedule_3x_entries(self, dados):
        """Estrategia de velas de 3x com timing preciso (MODULO 3)"""
        if not dados:
            return None
        current = dados[0]
    ...
    def schedule_10x_entries(self, dados):
        """Estrategia de velas de 10x com soma de minutos (MODULO 3)"""
        if not dados:
            return None
        current = dados[0]
        current_time = datetime.now()
    ...
    def detectar_padrao_3_sequencias(self, dados):
        """Detectar padrao de 3 sequencias (Roxo/Rosa/Quebra no Azul) - MODULO 4"""
        if len(dados) < 4:
            return None
    ...
    def detectar_gatilho_surreal(self, dados):
        """Detectar Gatilho Surreal (5+ velas abaixo de 1.49x) - MODULO 4"""
        if len(dados) < 6:
            return None
    ...
    def detectar_padrao_10x_reset(self, dados):
        """Detectar padrao 10x Reset (1.00x, 1.37x, 1.00x) - MODULO 4"""
        if len(dados) < 3:
            return None
    ...
    def detectar_repeticao_casas(self, dados):
        """Detectar repeticao de casas (1.30, 1.40, 1.70) - MODULO 4"""
        if len(dados) < 6:
            return None
    ...
    def gerar_regra_auto_correcao(self, contexto_erro, vela_entrada, vela_resultado):
        """Gerar regra especifica para evitar erro futuro"""
        regra = {
    ...
    def aplicar_filtro_auto_correcao(self, dados, sinal_proposto):
        """Aplicar todas as regras de auto-correcao antes de confirmar um sinal"""
    ...
    def calcular_bonus_horario(self, hora):
        """Calcula BONUS de confianca para horarios lucrativos"""
        horarios_lucrativos = {
            6: 5, 7: 5, 8: 4,
            13: 4,
            21: 5, 22: 6, 23: 5
    ...
    def gerenciar_banco(self):
        """Gerenciamento emocional com metas diarias (MODULO 1)"""
    ...
    def prever_valor_vela(self, padrao, dados):
        """Sistema de previsao de valor da vela"""
        previsoes_padroes = {
    ...
    def analise_completa_inteligente(self, dados, sequencias):
        """Analise super inteligente com DETECCAO AUTOMATICA de novos padroes + AUTO-CORRECAO - SEMPRE RETORNA 2 VALORES"""
        agora = datetime.now()
    ...
    def gerar_sinal_entrada(self, dados, sequencias):
        """GERAR SINAL PREDITIVO IMEDIATAMENTE apos deteccao (SEM ATRASO)"""
        agora = datetime.now()
    ...
    def obter_gatilhos_usados(self, sinal):
        """Obter lista de gatilhos usados no sinal - SEM LOSS_1_WIN, LOSS_2_WIN, LOSS_3_WIN"""
        gatilhos = []
    ...
    def obter_velas_mais_repetidas_24h(self):
        """Obter as 10 velas (multiplicadores) mais repetidas nas ultimas 24 horas"""
        agora = datetime.now()
    ...
    def exibir_status(self, dados, sequencias):
        """Exibir status em tempo real com TODAS as estatisticas"""
        os.system('clear')
    ...
    def executar(self, duracao_minutos=1440):
        """Executar bot com COLETA PRECISA de dados + operacao 24h"""
